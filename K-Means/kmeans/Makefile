...
# -----------------------------------------------------------
spark.root=/home/dheeraj/LargeScale/spark-2.3.4-bin-without-hadoop
hadoop.root=/home/dheeraj/LargeScale/hadoop-2.8.5
app.name=kmeans
jar.name=kmeans.jar
...
local.input=input
local.k=5
local.convergeDistance=0.5
local.output=output
# Pseudo-Cluster Execution
hdfs.user.name=dheeraj
hdfs.input=input
hdfs.output=output
# AWS EMR Execution
aws.emr.release=emr-5.26.0
aws.bucket.name=sparktwitterfollower1
aws.input=input
aws.output=output
aws.log.dir=log
aws.num.nodes=5
...
local: jar clean-local-output
	spark-submit --class ${job.name} --master ${local.master} --name "${app.name}" ${jar.name} ${local.input} ${local.k} ${local.convergeDistance} ${local.output}
...
aws: jar upload-app-aws delete-output-aws
	aws emr create-cluster \
		--name "TwitterFollower Spark Cluster" \
